...
try {
    $tables = $db->query("SELECT name FROM sqlite_master WHERE type='table' AND name='agents'");
    echo "<p><strong>Agents table exists:</strong> " . (count($tables) > 0 ? "YES" : "NO") . "</p>";
    
    if (count($tables) > 0) {
        // Get table structure
        $structure = $db->query("PRAGMA table_info(agents)");
        ...
        // Show all agents
        $agents = $db->query("SELECT * FROM agents LIMIT 20");
