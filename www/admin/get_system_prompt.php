...
try {
    ...
    // Check if claude_data table exists, create if not
    ...
    $result = $db->query("SELECT value FROM claude_data WHERE key = 'system_prompt'");
    
    if ($result && count($result) > 0) {
        echo json_encode(['success' => true, 'prompt' => $result[0]['value']]);
    } else {
        // Insert and return new default prompt
