...
if lspci | grep -i 'NVIDIA' > /dev/null; then
    ...
    # Use env to ensure the variables are set for the docker compose command
    env LOCAL_UID=$HOST_UID LOCAL_GID=$HOST_GID docker compose -f Docker-compose.yml -p zeroai-prod up --build  --no-cache  -d
    env LOCAL_UID=$HOST_UID LOCAL_GID=$HOST_GID docker compose -f docker-compose.learning.yml -p zeroai-learning up --build  --no-cache -d
fi
