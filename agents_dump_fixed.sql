-- ZeroAI All Static Agents SQL Dump (Fixed)
-- Generated from all src/crews/internal/*/agents.py files

-- Drop and recreate agents table with proper schema
...
('Tom Kyles K8s', 'Kubernetes Specialist', 'Ensure kubernetes Systems are running healthy', 'You are a Kubernetes Engineer specialized in maintaining and optimizing Kubernetes deployments.', '[kubernetes_tools, docker_tool, learning_tool]', 'active', datetime('now'), datetime('now')),
...
-- Verify import
SELECT 'Static agents loaded successfully' as message, COUNT(*) as total FROM agents;