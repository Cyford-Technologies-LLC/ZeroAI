...
def show_categories(metrics, outcomes):
    """Show category-specific performance."""
    if not metrics or not outcomes:
        console.print("❌ No learning data available", style="red")
        return

    console.print("\n🏷️ [bold blue]ZeroAI Category Performance[/bold blue]")
    console.print("=" * 60)

    for category, stats in metrics["categories"].items():
        console.print(f"\n📋 [bold]Details for category: {category}[/bold]")
        ...
        detail_table.add_row("Total tasks", str(stats["tasks"]))
        detail_table.add_row("Successful tasks", str(stats["successes"]))
        detail_table.add_row("Success rate", f"{success_rate:.2f}%")
        detail_table.add_row("Learning tokens earned", str(stats["tokens"]))

        console.print(detail_table)

        # Show model usage for this category
        if stats["models"]:
            ...
            model_table.add_column("Model", style="cyan")
            model_table.add_column("Tasks", style="white", justify="right")
            model_table.add_column("Percentage", style="green", justify="right")

            total_model_tasks = sum(stats["models"].values())

            for model, count in sorted(stats["models"].items(), key=lambda x: x[1], reverse=True):
                percentage = (count / total_model_tasks) * 100 if total_model_tasks > 0 else 0
                model_table.add_row(
                    model,
                    str(count),
                    f"{percentage:.2f}%"
            ...
            preferred_model = max(stats["models"].items(), key=lambda x: x[1])[0]
            console.print(f"Preferred model: [bold green]{preferred_model}[/bold green]")
...
def main():
    ...
    if args.action == "summary":
        ...
        show_categories(metrics, outcomes)
